...
class LikeListViewTests(APITestCase):
    def setUp(self):
        ...
        """
        User.objects.create_user(username="Lewis", password="password")
...
class LikeDetailViewTests(APITestCase):
    def setUp(self):
        ...
        Post.objects.create(
            owner=dave, title="post title3", description="test3", category="Greek"
        )
        Like.objects.create(owner=Lewis, post_id=2)  # 'id':1
